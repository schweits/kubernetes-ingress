...
A class of NGINX Ingress Controller.

A corresponding IngressClass resource with the name equal to the class must be deployed. Otherwise, NGINX Ingress Controller will fail to start.
NGINX Ingress Controller only processes resources that belong to its class - i.e. have the `ingressClassName` field resource equal to the class.

NGINX Ingress Controller processes all the resources that do not have the `ingressClassName` field.
